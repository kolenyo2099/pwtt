...
from .png_service import PWTT_VIS, RGB_VIS, render_triptych_and_panels
    ...
    summary = ee.Dictionary(
        {
            ...
            "damage_share_pct": ee.Number(damaged_area).divide(total_area).multiply(100),
...
def _validate_pwtt_coverage(aoi: ee.Geometry, parameters: dict[str, Any]) -> None:
    """Check for the Sentinel-1 inputs PWTT needs before calling the third-party code.
    ...
    turns that into a clear message the UI can show directly.
    """
    ...
    pre_orbits = set(pre_collection.aggregate_array("relativeOrbitNumber_start").distinct().getInfo() or [])
    post_orbits = set(post_collection.aggregate_array("relativeOrbitNumber_start").distinct().getInfo() or [])
...
def run_pwtt_image(aoi: ee.Geometry, parameters: dict[str, Any]) -> ee.Image:
    """Create the core PWTT image from saved run parameters."""

    _validate_pwtt_coverage(aoi, parameters)
...
def build_run_detail(run: dict[str, Any]) -> dict[str, Any]:
    """Attach fresh tile URLs to a stored run so the results page can render maps."""
    ...
    for layer_key, cache_file, panel in _preview_map:
...
def export_damage_kml(run: dict[str, Any]) -> dict[str, Any]:
    """Rebuild the damage mask and return KML-ready polygons."""
